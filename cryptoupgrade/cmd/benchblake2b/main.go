...
func main() {
	...
	if err := run(*rpcURL, *source, *solcPath, *soliditySource, *name, *mode, *inputText, *inputHex, *iterations, *warmup, *deployIterations, *upload, *algoGas, *upgradeDeployGas, *solidityDeployGas, *solidityAddress, *precompileAddress, *from); err != nil {
		fmt.Fprintf(os.Stderr, "benchmark failed: %v\n", err)
		os.Exit(1)
	}
}

func run(rpcURL, source, solcPath, soliditySource, name, mode, inputText, inputHex string, iterations, warmup, deployIterations int, upload bool, algoGas, upgradeDeployGas, solidityDeployGas uint64, solidityAddress, precompileAddress, fromText string) error {
	if iterations <= 0 {
	...
	blake2bABI, err := abi.JSON(strings.NewReader(blake2bABIJSON))
	if err != nil {
		return err
	}
	blake2bCallData, err := blake2bABI.Pack("sum256", input)
	if err != nil {
		return err
	}
	...
	if mode == "all" || mode == "deploy" || solidityAddress == "" {
		solidity, err = compileSolidity(solcPath, soliditySource)
		if err != nil {
			return err
		}
		fmt.Printf("solidity: source=%s bytecode=%d bytes solc=%s\n", mustAbs(soliditySource), len(solidity.Bin), solcPath)
	} else {
		solidity.ABI = blake2bABI
	}

	if mode == "all" || mode == "deploy" {
		if err := runDeploymentBenchmark(ctx, client, codeStorageABI, solidity.Bin, fromAddr, tmpDir, name, deployIterations, algoGas, upgradeDeployGas, solidityDeployGas); err != nil {
			return err
		}
	}
	if mode == "deploy" {
	...
	if solidityAddress != "" {
		...
		solidityAddr, deployResult, err = deploySolidityBlake2b(ctx, client, fromAddr, solidity.Bin, solidityDeployGas)
		if err != nil {
			return err
		}
	...
	solidityGas, err := estimateGas(ctx, client, txArgs{From: fromAddr, To: &solidityAddr, Data: blake2bCallData})
	if err != nil {
		return err
	}
	...
	solidityCall := func() ([32]byte, error) {
		out, err := ethCall(ctx, client, fromAddr, solidityAddr, blake2bCallData)
		if err != nil {
			return [32]byte{}, err
		}
		return unpackBytes32(bytes32Return, out)
	...
	upgradeStats, upgradeResult, err := benchmark("upgrade-callFunc-blake2b", iterations, warmup, upgrade)
	if err != nil {
		return err
	}
	solidityStats, solidityResult, err := benchmark("solidity-contract-blake2b", iterations, warmup, solidityCall)
	if err != nil {
		return err
	}
...
func runDeploymentBenchmark(ctx context.Context, client *rpc.Client, codeStorageABI abi.ABI, solidityBin []byte, from common.Address, tmpDir, baseName string, iterations int, algoGas, upgradeDeployGas, solidityDeployGas uint64) error {
	...
	for i := 0; i < iterations; i++ {
		...
		_, solidityResult, err := deploySolidityBlake2b(ctx, client, from, solidityBin, solidityDeployGas)
		if err != nil {
			return err
		}
	...
	printDeploySummary("deployment-upgrade-upload-blake2b", upgradeResults)
	printDeploySummary("deployment-solidity-contract-blake2b", solidityResults)
...
func compileSolidity(solcPath, sourcePath string) (compiledSolidity, error) {
	cmd := exec.Command(solcPath, "--optimize", "--via-ir", "--combined-json", "abi,bin", sourcePath)
	...
	var selectedABI json.RawMessage
	var selectedBin string
	found := false
	for name, contract := range combined.Contracts {
		if strings.HasSuffix(name, ":Blake2bSolidity") || !found {
			selectedABI = contract.ABI
			selectedBin = contract.Bin
			found = true
		}
		if strings.HasSuffix(name, ":Blake2bSolidity") {
			break
		}
	}
	if !found {
		return compiledSolidity{}, fmt.Errorf("no contract found in solc output")
	}
	contractABI, err := abi.JSON(bytes.NewReader(selectedABI))
	if err != nil {
		return compiledSolidity{}, err
	}
	bin, err := hex.DecodeString(selectedBin)
	if err != nil {
		return compiledSolidity{}, err
	}
	if len(bin) == 0 {
		return compiledSolidity{}, fmt.Errorf("solidity bytecode is empty")
	}
	return compiledSolidity{ABI: contractABI, Bin: bin}, nil
}

func deploySolidityBlake2b(ctx context.Context, client *rpc.Client, from common.Address, initCode []byte, gasLimit uint64) (common.Address, deployResult, error) {
	return deployContract(ctx, client, from, initCode, gasLimit, "solidity-contract-blake2b-deploy")
}
