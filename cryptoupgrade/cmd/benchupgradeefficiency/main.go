...
func parseConfig() (config, error) {
	...
	if cfg.resultDir == "" {
		cfg.resultDir = filepath.Join(cfg.repoRoot, "cryptoupgrade", "results", "upgrade-efficiency", "add-"+timestamp)
...
func newResult(cfg config) experimentResult {
	return experimentResult{
		...
		Activation: activationResult{
			BlockNumberSource:                  "inferred_from_upload_receipt",
			ObservationMethod:                  "CodeStorage.getInfo",
			ConfirmationAndActivationSamePoint: true,
		},
		...
		Limitations: []string{
			"当前 uploadCode 在交易执行期间同步 ActivateAlgorithm；公共 RPC 不暴露 plugin 编译完成或内存 map 写入的独立时间戳。",
			"confirmation 与 activation 在当前单节点 dev 链实现中是同一交易执行点，activation block 使用 upload receipt block 推断。",
			"confirm-to-activate observed latency 是 receipt 观测后首次 getInfo 成功的本地轮询间隔，不等同于链上独立激活耗时。",
			"validation 使用 eth_call，因此 validation block 表示 latest 调用上下文，不是验证交易所在区块。",
			"geth --dev 按需出块，submit-to-confirm latency 包含本地 dev miner 调度和 uploadCode 内部 plugin 编译时间。",
...
func run(ctx context.Context, cfg config, res *experimentResult) error {
	...
	if receipt.BlockNumber != nil {
		...
		res.Activation.BlockNumber = receipt.BlockNumber.Uint64()
	}
	...
	activationInfo, activationBlock, activationObserved, err := waitActivation(ctx, client, codeStorageABI, cfg, compressedPayload, 30*time.Second)
	if err != nil {
		return err
	}
	res.Activation.ObservationBlockNumber = activationBlock
	res.Activation.ObservedAt = activationObserved.Format(time.RFC3339Nano)
	res.Activation.Available = true
	res.Activation.CodeMatchesUploadPayload = activationInfo.code == compressedPayload
	res.Activation.Gas = activationInfo.gas
	res.Activation.InputType = activationInfo.itype
	res.Activation.OutputType = activationInfo.otype
	res.Latencies.ConfirmToActivateObservedMillis = millis(activationObserved.Sub(confirmationObserved))
	res.Latencies.ConfirmToActivateEffectiveMillis = 0

	validation, validationElapsed, err := validateAdd(ctx, client, codeStorageABI, from, cfg)
...
func compressSource(path string) (sourceBytes int, gzipBytes int, encoded string, err error) {
	...
	if _, err := io.Copy(gz, bytes.NewReader(source)); err != nil {
		return 0, 0, "", fmt.Errorf("gzip source: %w", err)
	}
	if err := gz.Close(); err != nil {
		return 0, 0, "", fmt.Errorf("close gzip source: %w", err)
	}
	return len(source), compressed.Len(), base64.StdEncoding.EncodeToString(compressed.Bytes()), nil
}
...
func waitActivation(ctx context.Context, client *rpc.Client, codeStorageABI abi.ABI, cfg config, uploadedCode string, timeout time.Duration) (activationInfo, uint64, time.Time, error) {
	data, err := codeStorageABI.Pack("getInfo", cfg.name)
	if err != nil {
		return activationInfo{}, 0, time.Time{}, err
	}
	deadline := time.Now().Add(timeout)
	var lastErr error
	for time.Now().Before(deadline) {
		raw, err := ethCall(ctx, client, common.Address{}, common.CodeStorageAddress, data)
		if err != nil {
			lastErr = err
			time.Sleep(200 * time.Millisecond)
			continue
		}
		values, err := codeStorageABI.Unpack("getInfo", raw)
		if err != nil {
			lastErr = err
			time.Sleep(200 * time.Millisecond)
			continue
		...
		info := activationInfo{
			...
			gas:   values[1].(uint64),
			itype: values[2].(string),
			otype: values[3].(string),
		}
		if info.code == uploadedCode && info.itype == cfg.inputType && info.otype == cfg.outputType {
			block, err := blockNumber(ctx, client)
			if err != nil {
				return activationInfo{}, 0, time.Time{}, err
			}
			return info, block, time.Now(), nil
		}
		lastErr = fmt.Errorf("getInfo returned gas=%d itype=%q otype=%q codeBytes=%d", info.gas, info.itype, info.otype, len(info.code))
		time.Sleep(200 * time.Millisecond)
	}
	if lastErr == nil {
		lastErr = errors.New("timeout")
	}
	return activationInfo{}, 0, time.Time{}, fmt.Errorf("activation observation failed: %w", lastErr)
}
