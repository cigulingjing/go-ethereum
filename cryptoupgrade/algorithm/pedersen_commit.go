...
// PedersenVerify recomputes the commitment and compares it with commitment.
func PedersenVerify(message, blinding, commitment []byte) bool {
	expected := PedersenCommit(message, blinding)
	return expected != nil && bytes.Equal(expected, commitment)
}
