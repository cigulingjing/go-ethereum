...
// SchnorrPublicKey derives a public key from a secret scalar.
func SchnorrPublicKey(secret []byte) []byte {
	p, q := schnorrGroup()
	x := schnorrScalar(secret, q)
	y := new(big.Int).Exp(big.NewInt(4), x, p)
	return schnorrFixedBytes(y)
}

// SchnorrProve returns publicKey || commitment || response for a message.
func SchnorrProve(secret, message []byte) []byte {
	p, q := schnorrGroup()
	x := schnorrScalar(secret, q)
	...
	out := make([]byte, 0, 3*schnorrFieldBytes())
	out = append(out, schnorrFixedBytes(y)...)
	out = append(out, schnorrFixedBytes(t)...)
	out = append(out, schnorrFixedBytes(s)...)
	return out
}

// SchnorrVerify verifies a proof created by SchnorrProve.
...
func schnorrScalar(data []byte, q *big.Int) *big.Int {
