...
```bash
experiments/cryptoupgrade/deployments/networks/local-2nodes.yaml
```
...
```bash
docker build \
  -f experiments/cryptoupgrade/deployments/docker/Dockerfile.lab \
...
```bash
sh experiments/cryptoupgrade/deployments/docker/check.sh
```
...
该测试要求 signer 账户已解锁。`validate` 会先完成纯网络验证，再由独立 smoke 包执行
`CodeStorage.uploadCode`；receipt 成功后继续重试 `CodeStorage.callFunc("Add", ...)`，
直到异步 bind-event activation 完成或超时。也可使用 `-mode smoke` 单独执行升级检查。
