...
func run() error {
	...
	if err := waitCallFuncReady(ctx, client, callArgs{From: from, To: &upgradeTo, Data: callData}, 30*time.Second); err != nil {
		return err
	}
...
func parseFlags() config {
	...
	flag.StringVar(&cfg.outputDir, "output-dir", "experiments/cryptoupgrade/results/real-chain", "directory used when -output-json is empty")
	flag.StringVar(&cfg.outputJSON, "output-json", "", "write full benchmark result JSON to this file; defaults under -output-dir")
	...
	if cfg.outputJSON == "" {
		...
		cfg.outputJSON = filepath.Join(cfg.outputDir, "run-"+timestamp, "result.json")
	}
	return cfg
}
...
func waitCallFuncReady(ctx context.Context, client *rpc.Client, args callArgs, timeout time.Duration) error {
	deadline := time.Now().Add(timeout)
	var lastErr error
	for time.Now().Before(deadline) {
		if _, err := ethCall(ctx, client, args); err == nil {
			return nil
		} else {
			lastErr = err
		}
